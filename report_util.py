...
> * Conf+ : 평균 긍정 확신도 
>   * 모델이 긍정으로 예측한 샘플들만 대상으로, 해당 샘플들의 긍정으로 분류할 확률값 평균 ( 긍정판단의 강도 )    
> * Conf- : 평균 부정 확신도 
>   * 모델이 부정으로 예측한 샘플들만 대상으로, 해당 샘플들의 부정으로 분류할 확률값 평균 ( 부정판단의 강도 )     
> * Margin : 평균 확률 마진 
>   * 긍정과 부정 확률의 차이로 값이 클수록 단호한 판단
> * P+Min/Med/Max : 긍정 확률 최소/중간/최대값   
> * P-Min/Med/Max : 부정 확률 최소/중간/최대값 
    """
    ...
    report_content += """
* 모델의 최종 선형 출력인 logit 값을 softmax 함수로 변환한 확률값 (모델이 긍정으로 판단할 확률)을 히스토그램으로 표현한 것 
    * 0.5 이상이면 긍정(1) 으로 분류됨 
* 비율(%) : 해당 확률 구간에 속하는 샘플 수가 전체 샘플 수에 대해 차지하는 백분율 
